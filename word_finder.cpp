...
void WordFinder::add(const wstring &str) {
    ...
    for(unsigned int i = 0U ; i < addStr.length() ; ++i) {
        wchar_t c = addStr.at(i);
        auto node = tmpNode->add(c);
        if (!node) {
            continue;
        }

        tmpNode = node;
        if (i == addStr.length() - 1U) {
            tmpNode->markEndNode();
        }
    }
}
...
IllegalWordChecker::IllegalWordChecker()
: _finder() {
}

IllegalWordChecker::IllegalWordChecker(const list<wstring> &strlist)
: _finder(strlist) {
}

void IllegalWordChecker::add(const wstring &str) {
    _finder.add(str);
}

wstring & IllegalWordChecker::filter(wstring &str) const {
    list<Word> list;
    for(const Word &word : _finder.find(list, str)) {
        str.replace(word.start, word.count(), word.count(), L'*');
    }
    return str;
}

bool IllegalWordChecker::check(const wstring &str) const {
    list<Word> list;
